...
def dsfdr(data,labels, method='meandiff', transform='rankdata', alpha=0.1, numperm=1000,fdrmethod='dsfdr'):
	...
	pvals=np.zeros([numbact])
	for crow in range(numbact):
		cvec=np.hstack([t[crow],u[crow,:]])
		cvec=1-(sp.stats.rankdata(cvec,method='min')/len(cvec))
		pvals[crow]=cvec[0]	
		t[crow]=cvec[0]	  # p-value for original test statistic t
		u[crow,:]=cvec[1:] # p-values for permutated test statistic u
	...
	if fdrmethod=='dsfdr':
		...
		# note: u and t here are p-values, so change np.sum(u>=trep) to np.sum(u<=trep)
		#import statsmodels.sandbox.stats.multicomp
